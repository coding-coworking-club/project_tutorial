...
class BaseballGameStatistics:
    ...
    # def get_pitch_counts(soup, status):
    #     pitches = soup.find("div", {"class","info"}).\
    #         find("div", {"class", status}).\
    #             findAll("span", {"class", re.compile("\s+active$")})
    #     return len(pitches)

    # def get_batting_result(soup):
    #     result = soup.find("div", {"class","info"}). \
    #         find("div", {"class", re.compile("batter_event\s+")}).get_text()
    #     return result    
