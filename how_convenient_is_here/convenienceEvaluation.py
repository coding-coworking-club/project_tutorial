...
class ConvenienceEvaluation():
    ...
    def __init__(self, address, key):
        ...
        address_prefix = "https://maps.googleapis.com/maps/api/geocode/json?address="
        address_key =  "&key=" + self.key
        address_url = address_prefix + self.address + address_key
        re = requests.get(address_url)
        # 中文url需要轉碼才能識别
    ...
    def dist_calculator(self, place_lat, place_lon):
        ...
        here_lat, here_lon, place_lat, place_lon = map(radians, [here_lat, here_lon, place_lat, place_lon])
        # haversine formula 
        diff_lat = place_lat - here_lat 
        diff_lon = place_lon - here_lon
        # 圓心角 Central angle
        angle = 2 * asin(sqrt(sin(diff_lat/2)**2 + cos(here_lon) * cos(place_lon) * sin(diff_lon/2)**2)) 
    ...
    def search(self, radius):
        ...
        for each_type in self.search_types:
            places_info = self.gmap_client.places_nearby(location = position, radius = radius, type = each_type)
            places_result += places_info["results"]
            count = 0
            while (count < 10 and places_info.get("next_page_token")):
                time.sleep(2)
                places_info = self.gmap_client.places_nearby(page_token = places_info["next_page_token"])
                places_result += places_info["results"]      
        self.places_table = pd.DataFrame(places_result)    
        geo_locate = self.places_table["geometry"].apply(lambda x: x["location"])
        self.places_table["distance"] = geo_locate.apply(lambda x: self.dist_calculator(x["lat"], x["lng"]))
        ...
        # self.places_table = self.places_table[["name", "distance", "vicinity", "rating", "latitude", "longitude"]]
    ...
    def get_point(self):
        ...
        for _type in self.grading_manual:
            _type_count = sum(self.places_table["types"].apply(lambda x: _type in x))
            _type_point = round(sum(self.places_table["types"].apply(lambda x: _type in x) * eval(self.grading_manual[_type])), 2)
